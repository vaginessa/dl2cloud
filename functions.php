...
function addtarget($target, $targetaccount, $data, $pdo)
{
 ...
 if ($target != '' && $targetaccount != '' && $data != '') {
  ...
  // Check
  $checkuserstmt->execute([$target, $targetaccount, $data]);
  ...
  $count = $checkuserstmt->rowCount();
   ...
   // do
   $success = $dost->execute([$target, $targetaccount, $data]);
         if (!$success) {
             return false;
         }
         switch ($target) {
...
function mvtarget($target, $targetaccount, $newtargetaccount, $pdo)
{
 ...
 if ($target != '' && $targetaccount != '' && $newtargetaccount != '') {
  ...
  // Check
  $checkuserstmt->execute([$target, $targetaccount, $newtargetaccount]);
  ...
  $count = $checkuserstmt->rowCount();
   ...
   // do
   $success = $dost->execute([$newtargetaccount, $target, $targetaccount]);
         if (!$success) {
             return false;
         }
     } else {
...
function create_user($username, $password, $email, $pdo)
{
 ...
 if ($username != '' && $password != '' && $email != '' && !eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
  ...
  // Check
  $checkuserstmt->execute([$username, $password, $random_salt, $token]);
  ...
  $count = $checkuserstmt->rowCount();
   ...
   // Let's do this!
   $result = $createuserstmt->execute([$username, $password, $random_salt, $token]);
         if (!$result) {
             return false;
         }
