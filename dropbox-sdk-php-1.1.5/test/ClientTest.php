...
class ClientTest extends PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        while ($result == null) {
            ...
            $i++;
            if ($i >= 100) {
                throw new Exception("Unable to create folder \"$basePath\"");
            }
            $result = $this->client->createFolder($basePath);
        }
    ...
    public function writeTempFile($size)
    {
        $fd = tmpfile();
    ...
    public function testDeltaPathPrefix()
    {
        // eat up all the deltas to the point where we should expect exactly
        // one after we add a new file
    ...
    public function testChunkedUploadWithSize()
    {
        $fd = $this->writeTempFile(1200);
        $contents = stream_get_contents($fd);
        fseek($fd, 0);
    ...
    public function testChunkedUploadWithFailures()
    {
        ...
        foreach (['lose-request', 'lose-response'] as $secondInstruction) {
            // Upload.
            $client->callCounter = 0;
            $client->instructions = ['ok', $secondInstruction];
            fseek($fd, 0);
        ...
        $client->callCounter = 0;
        $client->instructions =
            ['ok', 'lose-request', 'lose-request', 'lose-request', 'lose-response'];
        fseek($fd, 0);
        try {
